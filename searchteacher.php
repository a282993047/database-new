...
<div id="page">
    ...
    <?php
        ...
        $name = $_POST['name'];
        $name1=split(' ',$name);
        function makeSql($names){ //多关键字查询sql
            $sql = 'where TName ';
            if(!$names || empty($names)) return '';
            for($i=0;$i<count($names);$i++) {
                if($i==0) $sql.= "like '%$names[$i]%'";
                else $sql.= " or TName like '%$names[$i]%'";
            }
            return $sql;
         }

         $searchstr = makeSql($name1);

        $result = mysql_query("select * from Teachers LEFT JOIN Dept ON Teachers.Teacherid = Dept.Deptid $searchstr ");
    ...
    <div class="result">
        ...
        while($row = mysql_fetch_array($result)){
            ...
            if($row['Deptid'] == $row['Fatherid']) $dept =$row['DeptName'];
            else{
        ...
        <div class="result-box col-md-11">
            <form action="updateteacher.php" method="post">
                ...
                <div class="col-md-3" style="margin-top:10px; margin-bottom: 10px;">
                    ...
                    <button type="button" class="btn btn-primary" onclick="change(this)">修改</button>
...
//        dept.html("<input class='form-control' name='dept' value='"+deptval+"'>");
        ...
        <select class="form-control" name="dept" onchange="changexibie(this)">\
